...
fn main() -> BuilderResult<()> {
    ...
    println!("   Event ID: {}", section.splice_command.get_event_id().unwrap_or(0));
    ...
    println!("   Segmentation Event ID: {}", 
        section.splice_descriptors.first()
            .and_then(|d| match d {
                scte35_parsing::descriptors::SpliceDescriptor::Segmentation(seg) => 
    ...
    println!("   Descriptor Loop Length: {} bytes\n", section.descriptor_loop_length);
    ...
    println!("   Immediate splice: {:?}", 
        matches!(section.splice_command, scte35_parsing::types::SpliceCommand::SpliceInsert(ref si) 
    ...
    if let scte35_parsing::types::SpliceCommand::SpliceInsert(ref si) = section.splice_command {
        println!("   Program splice: {}", si.program_splice_flag == 1);
        println!("   Component count: {}", si.component_count);
        println!("   Components: {:?}", si.components.iter()
    ...
    if let Some(scte35_parsing::descriptors::SpliceDescriptor::Segmentation(ref seg)) = section.splice_descriptors.first() {
        println!("   Delivery restricted: {}", !seg.delivery_not_restricted_flag);
        println!("   Web delivery allowed: {:?}", seg.web_delivery_allowed_flag);
    ...
    match result {
        ...
        Err(BuilderError::InvalidUpidLength { expected, actual }) => {
            println!("   ✓ Caught invalid UPID length: expected {}, got {}", expected, actual);
        }
...
impl SpliceCommandExt for scte35_parsing::types::SpliceCommand {
    fn get_event_id(&self) -> Option<u32> {
        match self {
            scte35_parsing::types::SpliceCommand::SpliceInsert(si) => Some(si.splice_event_id),
            scte35_parsing::types::SpliceCommand::SpliceSchedule(ss) => Some(ss.splice_event_id),
            _ => None,
        }
    }
}